...
solvemail signs in with OAuth 2.0 through [fastgws](https://github.com/AnswerDotAI/fastgws), using the OAuth client and tokens that [gclientid](https://github.com/AnswerDotAI/gclientid) creates. Run `gclientid` once to provision a Google Cloud project and OAuth client, then `gclientid-auth you@example.com --preset gmail` to authorize an account. `oauth_creds(account='you@example.com')` loads the saved token, refreshes it when it expires, and runs the browser authorization again when it is missing or lacks a scope.

Create a [`Gmail`](https://AnswerDotAI.github.io/solvemail/core.html#gmail) client with the scopes you need: `readonly` to read and search, `modify` to also add and remove labels, or `full` for everything including permanent deletion. `profile()` returns the signed-in account, with the address on its `email` attribute.
...
`attachments` pulls a message’s files out as [`EmailAttachment`](https://AnswerDotAI.github.io/solvemail/core.html#emailattachment) objects, with `filename`, `mimeType`, and `body.size` available before you download anything. Each one is lazy: call `fetch()` to get the bytes, or pass `save=` a folder or filename to write it straight to disk.
...
A thread is a conversation: an email and its replies grouped under one id. `search_threads` returns [`Thread`](https://AnswerDotAI.github.io/solvemail/core.html#thread) objects, each holding its messages as [`Email`](https://AnswerDotAI.github.io/solvemail/core.html#email) objects on `.emails`, with `.last` for the most recent. Everything you know about an email works on each message in the thread.
